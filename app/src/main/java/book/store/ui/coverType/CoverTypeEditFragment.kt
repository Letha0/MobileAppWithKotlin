...
class CoverTypeEditFragment:Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        send_data.setOnClickListener {
            ...
            RetrofitClient.instance.editCoverType(session.TOKEN, session.ID, CoverTypeRequest(name))
                .enqueue(object : retrofit2.Callback<CoverType>{
                    ...
                    override fun onResponse(call: Call<CoverType>, response: Response<CoverType>) {
                        if(response.code()==200)
                            Toast.makeText(requireContext(), "Cover type edited!", Toast.LENGTH_SHORT).show()
                        else
